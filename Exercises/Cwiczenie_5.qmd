...
air_rec <- recipe(ozone ~., data = train_data) |> 
  update_role(date, so2, co, new_role = "predictor") |> 
...
Celem sprawdzenia obserwacji wykresu, ponawiam testy dla większych wartości tree_depth, gdyż widać że najwyższą wartośc osiągnał na max-ie, powinnysmy sprawdzić czy metryka dalej będzie rosła.

Dodatkowo zawężam cost_complexity do przedziału w około którego najlepiej się sprawdzał, celem wyciągniacia najlepszej wydajności modelu.

```{r}
set.seed(222)
#tworzymy nowe wartości dla siatki
parameters_net <- grid_regular(cost_complexity(range = c(-3.5,-2)),#skala log 
                       tree_depth(range = c(10L, 25L), trans = NULL), 
                       levels = 5)
...
Metryki okazują się być nieco gorsze od tych dobieranych automatycznie. Sprawdzamy czy dosżło do nadmiernego dopasowania:
...
Wnioski: Przy bardziej skomplikowanych danych wejściowych hiper-parametry mogą okazać się pomocne, jednak automatyczne funkcje dobierania nastaw są bardzo dobrze dostrojone i cięzko je poprawić metodą prób i błędów.
